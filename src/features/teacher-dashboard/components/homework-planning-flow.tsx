...
export function HomeworkPlanningFlow({ groups, presetGroupId, compact = false }: { groups: TeachingGroup[]; presetGroupId?: string; compact?: boolean }) {
  ...
  return <>
    ...
    {open ? <div className="teacher-modal-backdrop" role="presentation"><section className="teacher-flow-dialog" role="dialog" aria-modal="true" aria-labelledby="homework-flow-title">
      ...
      <footer className="teacher-flow-actions"><button className="teacher-button teacher-button-secondary" type="button" onClick={step === 1 ? close : () => setStep(step - 1)}>{step === 1 ? "Abbrechen" : "Zurück"}</button>{step < 3 ? <button className="teacher-button teacher-button-primary" type="button" onClick={() => setStep(step + 1)}>Weiter</button> : <button className="teacher-button teacher-button-primary" type="button" onClick={() => { setSaved(draft); close(); }}>Hausaufgabe speichern</button>}</footer>
    </section></div> : null}
  </>;
}
