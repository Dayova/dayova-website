...
export function AttendanceBoard({ students, initialRecords, date }: { students: Student[]; initialRecords: AttendanceRecord[]; date: string }) {
  const { attendance, saveAttendance } = useTeacherDashboard();
  const [records, setRecords] = useState(() => new Map(students.map((student) => {
    const existing = attendance.find((record) => record.studentId === student.id && record.date === date) ?? initialRecords.find((record) => record.studentId === student.id);
  ...
  return <div className="teacher-attendance-board">
    ...
    <div className="teacher-flow-actions"><button className="teacher-button teacher-button-primary" type="button" onClick={() => { saveAttendance([...records.values()]); setSaved(true); }}>Einträge speichern</button>{saved ? <span className="teacher-success-message" role="status">Klassenbuch wurde gespeichert und Risikosignale wurden aktualisiert.</span> : null}</div>
  </div>;
}
