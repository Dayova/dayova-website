...
export default async function ClassTeacherOverview({ params }: { params: Promise<{ classId: string }> }) {
  ...
  return <><BackButton fallback={`/teachers/classes/${classId}`} /><PageHeading title={`Klassenleitung · ${item.name}`} description="Überblick über Leistung, Mitarbeit, Anwesenheit und Unterstützungsbedarf." />
    ...
    <Panel title="Klassenübersicht" action={<TextAction href={`/teachers/classbook?class=${classId}`}>Klassenbuch öffnen</TextAction>}><div className="teacher-table-wrap"><table className="teacher-table"><thead><tr><th>Name</th><th>Durchschnitt</th><th>Hausaufgaben</th><th>Wissen</th><th>Status</th></tr></thead><tbody>{students.map((student) => <tr key={student.id}><td><TextAction href={`/teachers/students/${student.id}`}>{student.firstName} {student.lastName}</TextAction></td><td>{student.averageGrade}</td><td>{student.homeworkCompletionRate} %</td><td><ProgressBar value={student.knowledgeScore} /></td><td><RiskBadge risk={student.riskLevel} /></td></tr>)}</tbody></table></div></Panel>
  </>;
}
