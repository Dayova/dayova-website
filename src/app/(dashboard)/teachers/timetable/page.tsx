...
export default async function TimetablePage({
  ...
  searchParams: Promise<{ week?: string; woche?: string }>;
}) {
  const params = await searchParams;
  const requestedOffset = Number.parseInt(params.week ?? params.woche ?? "0", 10);
  ...
  return (
    <>
      <PageHeading
        ...
        actions={
          <PrimaryAction href="/teachers/assistant">
            Unterricht planen
          </PrimaryAction>
        }
      />
      ...
      <section className="teacher-timetable" aria-labelledby="timetable-week-heading">
        <header className="teacher-timetable-header">
          ...
          <nav className="teacher-timetable-navigation" aria-label="Schulwoche wechseln">
            <Link
              href={`/teachers/timetable?week=${weekOffset - 1}`}
              aria-label="Vorherige Woche"
            >
              <DayovaIcon icon={ArrowLeft01Icon} size={19} />
            </Link>
            {weekOffset !== 0 ? (
              <Link className="teacher-timetable-today" href="/teachers/timetable">
                Heute
              </Link>
            ) : null}
            <Link
                    ...
                    return (
                      <article className="teacher-timetable-entry" key={entry.id}>
                        ...
                        {entry.teachingGroupId ? (
                          <TextAction href={`/teachers/assistant?group=${entry.teachingGroupId}`}>
                            Vorbereiten
                          </TextAction>
                        ) : (
