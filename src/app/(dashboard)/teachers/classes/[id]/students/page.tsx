...
export default async function FullClassStudentsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><BackButton fallback={`/teachers/classes/${id}`} /><PageHeading title={`Schüler:innen der ${item.name}`} description="Vollständige Klassenliste mit Lernstand, Abschlussquote und Unterstützungsbedarf." />
