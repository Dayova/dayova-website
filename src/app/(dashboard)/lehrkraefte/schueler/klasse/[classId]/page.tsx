...
export default async function ClassStudentsPage({ params }: { params: Promise<{ classId: string }> }) {
  ...
  return <><BackButton fallback="/lehrkraefte/schueler" /><PageHeading title={`Schüler:innen · ${item.name}`} description={`${students.length} Schüler:innen · ${item.subjects.join(" · ")}`} />
