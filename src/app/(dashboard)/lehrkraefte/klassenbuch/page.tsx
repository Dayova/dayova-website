...
export default async function ClassBookPage({ searchParams }: { searchParams: Promise<{ klasse?: string; datum?: string; ansicht?: string }> }) {
  ...
  return <><PageHeading eyebrow="Klassenbereich" title={`Klassenbuch · ${selected.name}`} description="Dokumentieren Sie Anwesenheit und Krankmeldungen direkt im Unterricht." actions={<TextAction href="/lehrkraefte/klassenbuch">Klasse wechseln</TextAction>} /><ClassWorkspaceTabs active="klassenbuch" />
