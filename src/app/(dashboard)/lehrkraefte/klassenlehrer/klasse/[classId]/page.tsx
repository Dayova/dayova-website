...
export default async function ClassTeacherOverview({ params }: { params: Promise<{ classId: string }> }) {
  ...
  return <><BackButton fallback={`/lehrkraefte/klassen/${classId}`} /><PageHeading title={`Klassenleitung · ${item.name}`} description="Ganzheitlicher Überblick über Leistung, Mitarbeit, Anwesenheit und Unterstützungsbedarf." />
